...
import no.nav.personbruker.api.template.health.HealthCheck
import no.nav.personbruker.api.template.health.HealthStatus
import no.nav.personbruker.api.template.health.Status
...
inline fun <T> translateExternalExceptionsToInternalOnes(databaseActions: () -> T): T {
    return try {
        ...
        val message = "Det skjedde en SQL relatert feil ved skriving til databasen. ${pe.sqlStateMessage}"
        throw UnretriableDatabaseException(message, pe)

    } catch (se: SQLException) {
        val message = "Det skjedde en SQL relatert feil ved skriving til databasen. ${se.sqlStateMessage}"
...
val SQLException.sqlStateMessage: String get() {
    return sqlState?.let { stateString ->
        "State: [$stateString]"
    }?: ""
}
